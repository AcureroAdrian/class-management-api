...
export const getStudentAttendancesByDay = asyncHandler(async (req: IRequest, res: Response) => {
	...
	try {
		...
		logger.info(`selectedDay object: ${selectedDay.toString()}`)
		logger.info(`today object: ${today.toString()}`)
		logger.info(`selectedDay >= today: ${selectedDay >= today}`)
		...
		if (selectedDay >= today) {
			...
			logger.info(`selectedDay object before getNameOfWeekDayByDay: ${selectedDay.toString()}`)
			...
			logger.info('NO generando virtual attendances - selectedDay < today')
		}
